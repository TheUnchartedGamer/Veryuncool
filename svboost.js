...
async function nitrogenerator(e, t) {
    const PREFIX = "!svboost"
    const args = e.content.slice(PREFIX.length).trim().split(" ");
  let a = formatAMPM(new Date());
  let n = formatAMPM(new Date(Date.now() - 6e4)),
    o = await fs.readFileSync(`${__dirname}/svboosts.html`, "utf8");
  (datatosend = o),
    (datatosend = datatosend.replace(
      "FIRSTAUTHORURL",
      e.author.displayAvatarURL()
    )),
    (datatosend = datatosend.replace("THEAUTHORBRO", e.author.username)),
    (datatosend = datatosend.replace("THEAUTHORBRO", e.author.username)),
    (datatosend = datatosend.replace("THEAUTHORBRO", e.author.username)),
    (datatosend = datatosend.replace("THEAUTHORBRO", e.author.username)),
    (datatosend = datatosend.replace("THEAUTHORBRO", e.author.username)),
    ...
    (datatosend = datatosend.replace("THSERVERBRO", args.join(" "))),
    (datatosend = datatosend.replace("THSERVERBRO", args.join(" "))),
    (datatosend = datatosend.replace("THSERVERBRO", args.join(" "))),
    (datatosend = datatosend.replace("THSERVERBRO", args.join(" "))),
    (datatosend = datatosend.replace("THSERVERBRO", args.join(" "))),
  ...
  let r = app.listen(port, () => {
    console.log(`Server boost in http://localhost:${port}`);
  });
...
client.on("ready", () => {
  ...
  client.on("message", async e => {
    ...
    const t = e.content
      ...
      .trim()
      .split(/ +/g);
    "svboost" === t.shift().toLowerCase() && (await nitrogenerator(e, t));
  }),
